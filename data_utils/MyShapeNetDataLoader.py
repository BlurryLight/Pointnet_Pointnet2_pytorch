...
class PartNormalDataset(Dataset):
    def __init__(self, root_path = None,npoints=2500, split='train', normalize=True, jitter=False, class_choice=[]):
        ...
        self.part_num_dict = {k:len(v) for k,v in self.seg_classes.items()}
        self.part_num_sum = 50
        if len(class_choice) != 0:
            self.part_num_sum = 0
            self.seg_classes_new = {}
            for i in class_choice:
                self.seg_classes_new[i] = list(range(self.part_num_sum,self.part_num_sum+ self.part_num_dict[i]))
                self.part_num_sum += self.part_num_dict[i]
            self.seg_classes = self.seg_classes_new 
...
if __name__ == '__main__':
    ...
    d = PartNormalDataset(class_choice=['Chair'],split='train')
    print(len(d))
    d = PartNormalDataset(class_choice=['Chair','Airplane'],split='test')
    print(len(d))
    d = PartNormalDataset(split='test')
    print(len(d))
    # print(d.part_num_dict['Chair'])
    # print(d.part_num_dict['Airplane'])
    # print(d.part_num_sum)
    # ps,cls,seg,nom= d[1000]
    # print(ps[0:4],seg[0:4],cls)
    # print(d.get_classname(1000))
